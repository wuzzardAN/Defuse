...
public class InfoMenu : MonoBehaviour
{
	...
	void Start ()
	{
		...
		menuItems = new InfoMenuItems[itemsCount];
		for (int i = 0; i < itemsCount; i++) {
			// +1 to ignore the main button
			menuItems [i] = transform.GetChild (i + 1).GetComponent <InfoMenuItems> ();
		}
	...
	void ToggleMenu ()
	{
		...
		if (isExpanded) {
			...
			for (int i = 0; i < itemsCount; i++) {
				...
				coinText.DOFade(0, expandFadeDuration).From(1f);
				coinImage.DOFade(0, expandFadeDuration).From(1f);
			}
		} else {
			//menu closed
			for (int i = 0; i < itemsCount; i++) {
				...
				coinText.DOFade(1, expandFadeDuration).From(0f);
				coinImage.DOFade(1, expandFadeDuration).From(0f);
	...
	public void OnItemClick (int index)
	{
		...
		switch (index) {
			...
			case 1: 
				//second button
				Application.OpenURL("https://twitter.com/ControlGames7");
				break;
			case 2 :
				//third button
				Application.OpenURL("https://www.instagram.com/indie.control/");
				break;
		}
	}
